...
function buildServer() {
    ...
    server.registerTool("prepare_buy_order", {
        title: "Prepare a new BUY order (LP)",
        description: "For an LP creating a new express buy order. Validates inputs, checks for an existing " +
    ...
    server.registerTool("prepare_sell_order", {
        title: "Prepare a new SELL order (LP)",
        description: "For an LP creating a new express sell order. Same credential gating as " +
    ...
    server.registerTool("wait_for_payment_link", {
    ...
    server.registerTool("find_receipt", {
        title: "Find a payment receipt by order + taker",
        description: "Alternative receipt lookup for when you have the TrustExpress order address and the " +
    ...
    server.registerTool("wait_for_payment", {
        title: "Wait for a payment to settle",
        description: "Bounded poll (up to ~40s, checking every 4s) for a reservation to resolve. Call this " +
...
app.get("/pay/:reference", async (req, res) => {
    ...
    if (!intent)
        return res.status(404).json({ error: "Unknown or expired payment request" });
    res.json({
        label: "Trust Vault",
...
app.post("/pay/:reference", async (req, res) => {
    ...
    if (intent.kind === "create_buy_order") {
        ...
        transaction = await program.methods
            .createExpressBuyOrder(new BN(intent.seed), new BN(intent.amountRaw), new BN(intent.pricePerToken), Array.from(Buffer.from(intent.currency)), intent.paymentInstructions, intent.credentialId)
            .accounts(accounts)
            .transaction();
    }
        ...
        transaction = await program.methods
            .createExpressSell(new BN(intent.seed), new BN(intent.amountRaw), new BN(intent.pricePerToken), Array.from(Buffer.from(intent.currency)), intent.paymentInstructions, intent.credentialId)
            .accounts(accounts)
            .transaction();
    }
        ...
        transaction = await program.methods
            .instantSellReserve(new BN(intent.amountRaw), 0, // payment_mode: 0 = payment link
        ...
        transaction = await program.methods
            .instantReserve(new BN(intent.amountRaw), 0, // payment_mode -- UNCONFIRMED for this instruction, see caution above
        intent.payoutDetails, intent.payoutReference)
            .accounts(accounts)
            .transaction();
    }
    else {

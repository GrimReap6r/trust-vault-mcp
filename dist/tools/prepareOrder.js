...
export function deriveTrustExpressPda(maker, seed) {
    const seedBuf = Buffer.alloc(8);
    seedBuf.writeBigUInt64LE(seed);
    const [pda] = PublicKey.findProgramAddressSync([Buffer.from(TRUST_EXPRESS_SEED), maker.toBuffer(), seedBuf], PROGRAM_ID);
    return pda;
}
...
export async function prepareBuyOrder(args) {
    if (!SUPPORTED_CURRENCIES.includes(args.currency.toUpperCase())) {
    ...
    if (!credential) {
        ...
        return {
            ...
            message: "This wallet doesn't have a saved payment processor credential yet. " +

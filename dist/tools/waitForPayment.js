...
export async function waitForPayment(args) {
    ...
    while (Date.now() < deadline) {
        ...
        const stillPending = acc.reservedAmounts.some((r) => r.taker.toString() === args.takerWallet &&
        ...
        if (!stillPending) {
            ...
            return {
                outcome: "unknown",
                message: "The reservation is no longer active but I can't confirm whether it succeeded or was rejected/refunded from here. Please check your Trust Vault dashboard or bank app before treating this as paid.",
            };
        }

 ...
 */
function getEnumVariants(idl, typeName) {
    const typeDef = (idl.types ?? []).find((t) => t.name === typeName);
    if (!typeDef) {
        throw new Error(`IDL has no type named "${typeName}". Cannot safely decode this enum ` +
    ...
    return kind.variants.map((v) => toSnakeCase(v.name));
}
function toSnakeCase(name) {
    return name.replace(/([a-z0-9])([A-Z])/g, "$1_$2").toLowerCase();
}
export function decodeEscrowType(idl, escrowType) {
    const variants = getEnumVariants(idl, "EscrowType");
    const val = variants[escrowType];
    if (val !== "sell" && val !== "buy") {
        throw new Error(`IDL EscrowType variant at index ${escrowType} is "${val}", expected "sell" or "buy". ` +
            `The IDL's enum shape doesn't match what this decoder expects -- update decodeEscrowType.`);
    }
    return val;
}
export function decodeReservationStatus(idl, status) {
